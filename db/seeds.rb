...
File.open("db/movies.txt", "r") do |f|
  f.each_line do |line|
    ...
    title, country, year, image, director_id, type_id = line.chomp.split(";")
    Movie.create!(:title => title, :country => country, :year => year, :image => image, :director_id => director_id, :type_id => type_id)
  end
end
